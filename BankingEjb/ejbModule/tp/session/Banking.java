...
public class Banking implements BankingRemote {
	...
	public void populate() {
		...
		b.addCustomer(new Customer("Poulain", "Am�lie", "20, rue des mimosas",
				"34000"));
		...
		b = new Bank("Soci�t� G�niale", "1, parvis de la D�fonse", "92400",
				"5646456");
				...
				log(customer.toString());
				 ...
				 for (Account account : accounts) { //il faudrait utiliser les DAO et ne pas taper directement dans les entity
					 try {
						 ...
						 log("erreur pendant l'interrogation du compte, la banque "+bank.getName()+" vous pr�sente ses excuses");
					}
	...
	public void test()  {
		...
		populate();
	    addBankName("cr�dit truc");
	    addBankName("cr�dit muche");
		...
		log("Liste des communes apr�s la suppression de la seconde");
		...
		if (getCustomers().size() > 0) {
			log("Liste des clients apr�s la suppression");
		...
		log("Liste des banques apr�s la suppression");
		banks = getBanks();
		if (banks != null) {
			for (Bank bank : banks)
				log(bank.getName());
		...
		customers = getCustomers();
		if (customers.size() > 0) {
			log("Liste des clients apr�s la suppression de la banque");
			if (customers != null) {
				for (Customer Customer : customers) {
					log(Customer.getName());
				}
	...
	public void delete(Bank b) {
		// TODO v�rifier si c'est n�cessaire de supprimer les customers d'une
		// banque

		b = em.find(Bank.class, b.getId());
		/*
		 * Collection<Customer> bcusts = b.getCustomers(); //est-ce n�cessaire ??
